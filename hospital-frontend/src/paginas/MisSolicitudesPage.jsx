...
const MisSolicitudesPage = () => {
  ...
  const [estadisticas, setEstadisticas] = useState({
    total: 0, pendientes: 0, enProceso: 0, completadas: 0
  });
  ...
  const calcularEstadisticas = (data) => {
    ...
    setEstadisticas({
      ...
      pendientes: data.filter(s =>
        ['pendiente_solicitante', 'pendiente_jefe_seccion', 'pendiente_jefe_activos', 'pendiente_soporte'].includes(s.estado)
      ).length,
      enProceso: data.filter(s =>
        ['asignado', 'en_proceso', 'pendiente_conformacion', 'pendiente_jefe_mantenimiento'].includes(s.estado)
      ).length,
      completadas: data.filter(s => s.estado === 'completado').length,
    });
  };

  const cargarMisSolicitudes = useCallback(async (silencioso = false) => {
    try {
  ...
  // 🔴 Auto-refresh silencioso al recibir notificaciones (sin toast molesto,
  // sin loader, solo actualiza los datos en segundo plano)
  useAutoRefresh(() => cargarMisSolicitudes(true));
    ...
    <div className="mis-solicitudes-page">
      ...
      <div className="filtros-container">
        <div className="filtros-scroll">
          <button className={`filtro-chip ${filtroActivo === 'todas' ? 'active' : ''}`} onClick={() => setFiltroActivo('todas')}>Todas</button>
          <button className={`filtro-chip ${filtroActivo === 'pendientes' ? 'active' : ''}`} onClick={() => setFiltroActivo('pendientes')}>Pendientes</button>
          <button className={`filtro-chip ${filtroActivo === 'en_proceso' ? 'active' : ''}`} onClick={() => setFiltroActivo('en_proceso')}>En Proceso</button>
          <button className={`filtro-chip ${filtroActivo === 'completadas' ? 'active' : ''}`} onClick={() => setFiltroActivo('completadas')}>Completadas</button>
        </div>
      </div>
      ...
      <div className="solicitudes-list">
        {solicitudesFiltradas.length === 0 ? (
          ...
          solicitudesFiltradas.map((solicitud) => {
            ...
            return (
              <div key={solicitud.id} className="solicitud-card" onClick={() => handleVerDetalle(solicitud.id)}>
                <div className="card-header">
                  <div className="card-title-section">
                    <span className="solicitud-id">#{solicitud.id}</span>
                    <h3 className="solicitud-titulo">{solicitud.titulo}</h3>
                  </div>
                  <div className="estado-badge" style={{ backgroundColor: estadoConfig.color + '20', color: estadoConfig.color }}>
                ...
                <div className="card-body">
                  ...
                  <div className="solicitud-meta">
                    ...
                    <div className="meta-item"><span className="meta-icon">📅</span><span>{formatearFecha(solicitud.creado_en)}</span></div>
                  </div>
                  {solicitud.tipo_solicitud === 'con_material' && (
                    <div className="tipo-badge material"><span>📦</span><span>Se usaron materiales</span></div>
                  )}
                </div>

                <div className="card-footer">
                  <button className="btn-ver-detalle">Ver detalles <span className="arrow">→</span></button>
                </div>
              </div>
            );
          })
      ...
      <button className="fab-nueva-solicitud" onClick={() => navigate('/nueva-solicitud')} aria-label="Nueva solicitud">
        <FiPlus size={24} />
      </button>
    </div>
  );
